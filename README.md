...
<head>
    ...
    <title>Business & Perspective Lab</title>
    <style>
        :root {
        ...
        .gap-input { border: none; border-bottom: 2px solid var(--secondary-color); width: 160px; text-align: center; font-size: inherit; outline: none; background: transparent; color: var(--accent-color); font-weight: bold; }
        ...
        .match-box.selected { border-color: var(--secondary-color); background: #eef7ff; }
...
<header>
    <h1>Perspective & Strategy Lab</h1>
...
<div class="container">
    ...
    <div id="tab-0" class="exercise-section">
        <h2>📚 American English Study List</h2>
...
<script>
    const data = [
        { word: "used to", def: "Accustomed to something through experience.", sents: ["I am ___ getting up at 6 AM.", "She isn't ___ the hot weather yet."] },
        { word: "since", def: "From a particular time in the past until now.", sents: ["I have lived here ___ 2015.", "He hasn't called me ___ last week."] },
        { word: "lonely", def: "Unhappy because one has no friends or company.", sents: ["Moving to a new city can be ___.", "I felt very ___ when my friends left."] },
        { word: "mainland", def: "A large continuous extent of land (not an island).", sents: ["We took a ferry from the island to the ___.", "Life on the ___ is much faster."] },
        { word: "holistic view", def: "Understanding a system as a whole, not just parts.", sents: ["A CEO needs a ___ of the company.", "Our strategy provides a ___ of the market."] },
        { word: "undertake", def: "To commit oneself to and begin (a task/project).", sents: ["The company will ___ a new project.", "It's a big task to ___ alone."] }
    ];
    ...
    function init() {
        ...
        shuffle(wPool);
        document.getElementById('writing-content').innerHTML = wPool.slice(0, 8).map((s, i) => `<div class="gap-sentence">${s.t.replace(s.a, '___').replace('___', `<input type="text" class="gap-input" data-ans="${s.a}" id="w-${i}">`)}<div id="fb-w-${i}"></div></div>`).join('');
        ...
        shuffle(pPool);
        document.getElementById('pron-content').innerHTML = pPool.map((s, i) => `<div class="gap-sentence">${s.t.replace(s.a, '_________')} <button class="mic-btn" onclick="testSpeech('${s.a}', 'p-fb-${i}')">🎤</button><div id="p-fb-${i}" style="font-size: 0.85em; color:#888; margin-top:5px;">Identify from the bank & speak!</div></div>`).join('');
    }
